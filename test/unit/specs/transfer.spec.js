...
describe('Transfer', () => {
  ...
  it('filterable', (done) => {
    ...
    const leftList = transfer.$el.querySelector('.ac-transfer-panel').__vue__;
    leftList.query = '1';
    setTimeout((_) => {
  ...
  it('customize', () => {
    ...
    const label = transfer.querySelector(
      '.ac-transfer-panel__header .ac-checkbox__label'
    );
    expect(label.innerText.indexOf('表1') > -1).to.true;
    expect(
      transfer.querySelector(
        '.ac-transfer-panel__list .ac-checkbox__label span'
      ).innerText
    ).to.equal('1 - 备选项 1');
  ...
  it('check', () => {
    ...
    const leftList =
      vm.$refs.transfer.$el.querySelector('.ac-transfer-panel').__vue__;
        ...
        setTimeout(() => {
          const targetItems = [].slice.call(
            vm.$el.querySelectorAll(
              '.ac-transfer__buttons + .ac-transfer-panel .ac-transfer-panel__body .ac-checkbox__label span'
            )
          );
