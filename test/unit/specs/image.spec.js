...
describe('Image', () => {
  ...
  it('create', async () => {
    ...
    const placeholder = vm.$el.querySelector('.ac-image__placeholder');
    const error = vm.$el.querySelector('.ac-image__error');
    let img = vm.$el.querySelector('.ac-image__inner');
    expect(placeholder).to.exist;
    expect(error).to.be.null;
    expect(img).to.be.null;

    await wait();
    img = vm.$el.querySelector('.ac-image__inner');
  ...
  it('$attrs', async () => {
    ...
    await wait();
    const $img = vm.$el.querySelector('.ac-image__inner');
  ...
  it('pass event listeners', async () => {
    ...
    await wait();
    vm.$el.querySelector('.ac-image__inner').click();
    await wait();
  ...
  it('big image preview', async () => {
    ...
    expect(document.querySelector('.ac-image-viewer__wrapper')).to.exist;
    document.querySelector('.ac-image-viewer__close').click();
    await wait(1000);
    expect(document.querySelector('.ac-image-viewer__wrapper')).to.not.exist;
  });
});
