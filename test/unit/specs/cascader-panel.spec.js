...
const getValidOptions = (el, menuIndex) =>
  getMenus(el)[menuIndex].querySelectorAll('.ac-cascader-node[tabindex="-1"]');
const getLabel = (el) =>
  el.querySelector('.ac-cascader-node__label').textContent;
...
describe('CascaderPanel', () => {
  ...
  it('with default value', async () => {
    ...
    expect(getOptions(el, 2)[0].querySelector('.ac-icon-check')).to.exist;
  });
  ...
  it('multiple mode', async () => {
    ...
    const el = vm.$el;
    const checkbox = getOptions(el, 0)[0].querySelector('.ac-checkbox');
    expect(checkbox).to.exist;
    expect(
      checkbox.querySelector('.ac-checkbox__input').className
    ).to.not.includes('is-checked');
    checkbox.querySelector('input').click();

    await waitImmediate();
    expect(checkbox.querySelector('.ac-checkbox__input').className).to.includes(
      'is-checked'
    );
  ...
  it('check strictly in single mode', async () => {
    ...
    const el = vm.$el;
    const radio = getOptions(el, 0)[0].querySelector('.ac-radio');
    ...
    expect(
      getOptions(el, 0)[1].querySelector('.ac-radio').className
  ...
  it('check strictly in multiple mode', async () => {
    ...
    await waitImmediate();
    expect(
      getOptions(el, 1)[0].querySelector('.ac-checkbox').className
    ).to.not.includes('is-checked');
    getOptions(el, 1)[0]
      .querySelector('input')
      .click();
    ...
    expect(vm.value).to.deep.equal(['zhejiang', 'hangzhou']);
    expect(
      getOptions(el, 0)[1].querySelector('.ac-checkbox').className
    ).to.includes('is-disabled');
  });
