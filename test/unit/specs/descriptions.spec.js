...
describe('Descriptions', () => {
  ...
  it('render test', () => {
    ...
    const el = vm.$el;
    expect(el.querySelector('.ac-descriptions__title').textContent).to.equal(
      'title'
    );
    expect(el.querySelector('.ac-descriptions__extra').textContent).to.equal(
      'extra'
    );
    expect(el.querySelectorAll('.ac-descriptions-item__label').length).to.equal(
      4
    );
  });
  ...
  it('should render border props', () => {
    ...
    const el = vm.$el;
    expect(Array.from(el.querySelector('table').classList)).to.contain(
      'is-bordered'
    );
  });
  ...
  it('should render direction props', async () => {
    ...
    expect(vm.$el.querySelector('tr').children.length).to.equal(10);
    expect(vm.$el.querySelectorAll('tr')[0].children[0].innerHTML).to.equal(
      vm.$el.querySelectorAll('tr')[0].children[1].innerHTML
    );
    vm.direction = 'vertical';
    await waitImmediate();
    expect(vm.$el.querySelector('tr').children.length).to.equal(5);
    expect(vm.$el.querySelectorAll('tr')[0].children[0].innerHTML).to.equal(
      vm.$el.querySelectorAll('tr')[1].children[0].innerHTML
    );
  });
  ...
  it('should render title slots', async () => {
    ...
    expect(vm.$el.querySelector('.ac-descriptions__title').innerText).to.equal(
      'title'
    );
  });
  ...
  it('should render span props', async () => {
    ...
    expect(vm.$el.querySelectorAll('td')[1].getAttribute('colSpan')).to.equal(
      '2'
    );
  });
  ...
  it('re-rendered when slots is updated', async () => {
    ...
    await waitImmediate();
    expect(vm.$el.querySelector('.ac-tag').innerText).to.equal(CHANGE_VALUE);
  });
});
