...
describe('Carousel', () => {
      ...
      template: `
        <div>
          <el-carousel ref="carousel">
            <el-carousac-item v-for="item in 3" :key="item"></el-carousac-item>
          </el-carousel>
        </div>
      `,
    });
    expect(vm.$refs.carousel.direction).to.be.equal('horizontal');
    expect(vm.$el.querySelectorAll('.ac-carousel__item').length).to.equal(3);
  });
  ...
  it('auto play', (done) => {
    ...
    setTimeout(() => {
      const items = vm.$el.querySelectorAll('.ac-carousel__item');
  ...
  it('initial index', (done) => {
    ...
    setTimeout(() => {
      expect(
        vm.$el
          .querySelectorAll('.ac-carousel__item')[1]
  ...
  it('label', (done) => {
    vm = createVue({
      template: `
        <div>
          <el-carousel>
            <el-carousac-item v-for="item in 3" :key="item" :label="item"></el-carousac-item>
          </el-carousel>
        </div>
      `,
    });
    setTimeout((_) => {
      expect(vm.$el.querySelector('.ac-carousel__button').innerText).to.equal(
        '1'
      );
      done();
    }, 10);
  ...
  describe('manual control', () => {
    ...
    it('click', (done) => {
      ...
      setTimeout(() => {
        ...
        vm.$el.querySelectorAll('.ac-carousel__indicator')[2].click();
        setTimeout(() => {
          expect(items[2].classList.contains('is-active')).to.true;
          carousel.handleButtonEnter('right');
          vm.$el.querySelector('.ac-carousel__arrow--right').click();
          setTimeout(() => {
            expect(items[0].classList.contains('is-active')).to.true;
            done();
          }, 10);
  ...
  it('card', (done) => {
      ...
      template: `
        <div>
          <el-carousel :autoplay="false" type="card">
            <el-carousac-item v-for="item in 7" :key="item"></el-carousac-item>
          </el-carousel>
        </div>
      `,
    });
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        expect(items[1].classList.contains('is-active')).to.true;
        vm.$el.querySelector('.ac-carousel__arrow--left').click();
        setTimeout(() => {
          expect(items[0].classList.contains('is-active')).to.true;
          items[6].click();
          setTimeout(() => {
