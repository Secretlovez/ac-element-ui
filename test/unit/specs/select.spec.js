...
describe('Select', () => {
  ...
  it('create', () => {
    ...
    expect(vm.$el.className).to.equal('ac-select');
    expect(vm.$el.querySelector('.ac-input__inner').placeholder).to.equal(
      '请选择'
    );
    vm.toggleMenu();
  ...
  it('options rendered correctly', () => {
    vm = getSelectVm();
    const options = vm.$el.querySelectorAll('.ac-select-dropdown__item');
  ...
  it('custom dropdown class', () => {
    ...
    const dropdown = vm.$el.querySelector('.ac-select-dropdown');
  ...
  it('default value', (done) => {
    ...
    setTimeout(() => {
      expect(vm.$el.querySelector('.ac-input__inner').value).to.equal('双皮奶');
      done();
    }, 100);
  });
  ...
  it('disabled select', () => {
    ...
    expect(vm.$el.querySelector('.ac-input').classList.contains('is-disabled'))
      .to.true;
  });
  ...
  it('clearable', (done) => {
    ...
    setTimeout(() => {
      const iconClear = vm.$el.querySelector(
        '.ac-input__icon.ac-icon-circle-close'
      );
  ...
  it('object typed value', (done) => {
    ...
    setTimeout(() => {
      expect(vm.$el.querySelector('.ac-input__inner').value).to.equal('label1');
      expect(
        vm.$el
          .querySelector('.ac-select-dropdown__item')
  ...
  it('prefixed icon', () => {
      ...
      template: `
        <div>
          <el-select v-model="value">
            ...
            <i slot="prefix" class="ac-input__icon ac-icon-search"></i>
          </el-select>
        </div>
      `,
    ...
    expect(
      vm.$el
        .querySelector('.ac-input__icon')
        .classList.contains('ac-icon-search')
    ).to.be.true;
  });

  it('custom ac-option template', () => {
    vm = createVue(
      {
        template: `
        <div>
    ...
    expect(
      vm.$el.querySelector('.ac-select-dropdown__item p').textContent
  ...
  it('option group', () => {
    ...
    const groups = vm.$el.querySelectorAll('.ac-select-group__wrap');
    const options = groups[1].querySelectorAll('.ac-select-dropdown__item');
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          const tagCloseIcons = vm.$el.querySelectorAll('.ac-tag__close');
  ...
  it('should return focus to input inside select after option select', (done) => {
    ...
    const option = vm.$el.querySelectorAll('.ac-select-dropdown__item')[0];
  ...
  it('should set placeholder to label of selected option when filterable is true and multiple is false', async () => {
    ...
    expect(vm.$el.querySelector('.ac-input__inner').placeholder).to.be.equal(
      'test'
    );
  ...
  it('default value is null or undefined', async () => {
    ...
    vm.value = null;
    await waitImmediate();
    expect(vm.$el.querySelector('.ac-input__inner').value).to.equal('');
    vm.value = '选项1';
    await waitImmediate();
    expect(vm.$el.querySelector('.ac-input__inner').value).to.equal('黄金糕');
  });
