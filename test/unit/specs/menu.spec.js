...
describe('Menu', () => {
  ...
  describe('submenu', function() {
    it('toggle', (done) => {
      ...
      submenu.$el.querySelector('.ac-submenu__title').click();
      setTimeout((_) => {
  ...
  it('unique-opened', (done) => {
    ...
    vm.$refs.submenu2.$el.querySelector('.ac-submenu__title').click();
    setTimeout((_) => {
  ...
  it('horizontal mode', (done) => {
    ...
    expect(vm.$el.classList.contains('ac-menu--horizontal')).to.be.true;
    var submenu = vm.$refs.submenu;
    triggerEvent(submenu.$el, 'mouseenter');
    setTimeout((_) => {
      expect(
        document.body.querySelector('.ac-menu--popup').parentElement.style
          .display
      ).to.not.ok;
      done();
    }, 500);
  ...
  it('menu trigger click', (done) => {
    ...
    expect(vm.$el.classList.contains('ac-menu--horizontal')).to.be.true;
    var submenu = vm.$refs.submenu;
    var triggerElm = submenu.$el.querySelector('.ac-submenu__title');
  ...
  it('menu group', () => {
    vm = createVue(
      {
        ...
        <el-menu mode="vertical" default-active="1">
          <el-menu-item-group title="分组一" ref="group1">
            <el-menu-item index="1"><i class="ac-icon-message"></i>导航一</el-menu-item>
            <el-menu-item index="2"><i class="ac-icon-message"></i>导航二</el-menu-item>
          </el-menu-item-group>
          <el-submenu index="5">
    ...
    expect(
      vm.$refs.group1.$el.querySelector('.ac-menu-item-group__title').innerText
    ).to.be.equal('分组一');
  });
  it('dynamic menus, issue 9092', (done) => {
    ...
    setTimeout((_) => {
      ...
      setTimeout((_) => {
        expect(
          vm.$el.querySelector('.ac-menu-item.is-active').innerText
        ).to.equal('new');
        done();
      }, 20);
    }, 100);
