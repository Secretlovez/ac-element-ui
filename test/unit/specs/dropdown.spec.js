...
describe('Dropdown', () => {
    ...
    vm = createVue(
      {
        ...
        <el-dropdown ref="dropdown">
          <span class="ac-dropdown-link">
            下拉菜单<i class="ac-icon-caret-bottom ac-icon-right"></i>
          </span>
    ...
    vm = createVue(
      {
        ...
        <el-dropdown trigger="click" ref="dropdown">
          <span class="ac-dropdown-link">
            下拉菜单trigger click<i class="ac-icon-caret-bottom ac-icon-right"></i>
          </span>
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item>黄金糕</el-dropdown-item>
  ...
  it('split button', (done) => {
    ...
    let dropdown = vm.$refs.dropdown;
    let dropdownElm = dropdown.$el;
    let triggerElm = dropdownElm.querySelector('.ac-dropdown__caret-button');
    var callback = sinon.spy();

    dropdown.$on('click', callback);
    dropdownElm.querySelector('.ac-button').click();
  ...
  it('updatePopper', (done) => {
    ...
    triggerEvent(triggerElm, 'mouseenter');
    setTimeout(() => {
      const zIndex1 = document.querySelector('.ac-dropdown-menu').style.zIndex;
      dropdown.broadcast('ElDropdownMenu', 'updatePopper');
      setTimeout(() => {
        const zIndex2 = document.querySelector('.ac-dropdown-menu').style
          .zIndex;
  ...
  it('dropdown disabled', (done) => {
    ...
    setTimeout(() => {
      expect(
        dropdownElm.querySelectorAll('.ac-dropdown-link[disabled]').length
      ).equal(1);
      expect(dropdown.visible).to.be.false;
      done();
    }, 10);
