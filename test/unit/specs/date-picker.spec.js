...
describe('DatePicker', () => {
  ...
  it('select date', (done) => {
    ...
    setTimeout((_) => {
      ...
      const spans = $el.querySelectorAll('.ac-date-picker__header-label');
      const arrowLeftElm = $el.querySelector(
        '.ac-date-picker__prev-btn.ac-icon-arrow-left'
      );
      const arrowRightElm = $el.querySelector(
        '.ac-date-picker__next-btn.ac-icon-arrow-right'
      );

      expect(spans[0].textContent).to.include(date.getFullYear());
      expect(spans[1].textContent).to.include(date.getMonth() + 1);
      $el
        .querySelector('.ac-date-picker__prev-btn.ac-icon-d-arrow-left')
        .click();
      let count = 20;
      while (--count) {
        arrowLeftElm.click();
  ...
  it('disabled clear value', (done) => {
    ...
    setTimeout((_) => {
      ...
      $el.querySelector('td.available').click();
      vm.$nextTick((_) => {
        vm.$el.querySelector('.ac-input__icon').click();
        setTimeout((_) => {
  ...
  it('reset', (done) => {
    ...
    setTimeout((_) => {
      const $el = vm.$refs.compo.picker.$el;
      $el
        .querySelector('.ac-date-picker__next-btn.ac-icon-arrow-right')
        .click();
      setTimeout((_) => {
        $el.querySelector('td.available').click();
        vm.$nextTick((_) => {
  ...
  describe('value-format', () => {
    ...
    it('emits', (done) => {
      ...
      setTimeout((_) => {
        vm.$refs.compo.picker.$el
          .querySelector('.ac-date-table td.available')
          .click();
        setTimeout((_) => {
    ...
    it('accepts', (done) => {
      ...
      setTimeout((_) => {
        ...
        vm.$refs.compo.picker.$el
          .querySelector('.ac-date-table .current')
          .click();
        setTimeout((_) => {
          expect(input.value).to.equal('2000-02-01');
          done();
  ...
  describe('default value', () => {
    ...
    it('it works', (done) => {
      ...
      setTimeout((_) => {
        const $el = vm.$refs.compo.picker.$el;
        expect($el.querySelector('.ac-month-table td.default')).to.exist;
        expect($el.querySelector('.ac-year-table td.default')).to.exist;
        const defaultEls = $el.querySelectorAll('.ac-date-table td.default');
    ...
    it('is reactive, works with clear', (done) => {
      ...
      setTimeout((_) => {
        ...
        expect($el.querySelector('.ac-date-table td.current')).to.exist;
        vm.defaultValue = defaultValue;
        triggerEvent(vm.$refs.compo.$el, 'mouseenter');
        setTimeout((_) => {
          vm.$el.querySelector('.ac-icon-circle-close').click();
          setTimeout((_) => {
            input.focus();
            setTimeout(() => {
              $el = vm.$refs.compo.picker.$el;
              expect($el.querySelector('.ac-date-table td.current')).to.not
                .exist;
              expect($el.querySelector('.ac-date-table td.default')).to.exist;
              $el.querySelector('.ac-date-table td.default').click();
              setTimeout(() => {
                expect(vm.value).to.eql(expectValue);
                done();
              }, DELAY);
  ...
  describe('navigation', () => {
    ...
    const initNavigationTest = async (value) => {
      ...
      await wait();
      const $el = vm.$refs.compo.picker.$el;
      prevMonth = $el.querySelector('button.ac-icon-arrow-left');
      prevYear = $el.querySelector('button.ac-icon-d-arrow-left');
      nextMonth = $el.querySelector('button.ac-icon-arrow-right');
      nextYear = $el.querySelector('button.ac-icon-d-arrow-right');
      getYearLabel = () =>
        $el.querySelectorAll('.ac-date-picker__header-label')[0].textContent;
      getMonthLabel = () =>
        $el.querySelectorAll('.ac-date-picker__header-label')[1].textContent;
    };
    ...
    it('month label with fewer dates', async () => {
      ...
      const monthLabel = $el.querySelectorAll(
        '.ac-date-picker__header-label'
      )[1];
      await clickAndWait(monthLabel);
      const juneLabel = $el.querySelectorAll('.ac-month-table td a')[5];
  ...
  it('type:month', (done) => {
    ...
    setTimeout((_) => {
      expect(vm.picker.$el.querySelector('.ac-month-table').style.display).to.be
        .empty;
      expect(
        vm.picker.$el.querySelector('.ac-year-table').style.display
      ).to.be.equal('none');
      vm.picker.$el.querySelector('.ac-month-table a.cell').click();
  ...
  it('type:year', (done) => {
    ...
    setTimeout((_) => {
      expect(vm.picker.$el.querySelector('.ac-year-table').style.display).to
        .empty;
      expect(
        vm.picker.$el.querySelector('.ac-month-table').style.display
      ).to.be.equal('none');

      const leftBtn = vm.picker.$el.querySelector('.ac-icon-d-arrow-left');
      const rightBtn = vm.picker.$el.querySelector('.ac-icon-d-arrow-right');
      let count = 20;
      ...
      vm.picker.$el.querySelector('.ac-year-table a.cell').click();
      expect(vm.pickerVisible).to.false;
      done();
    }, DELAY);
  });
  ...
  describe('type:datetime', () => {
    ...
    it('create', (done) => {
      ...
      setTimeout((_) => {
        expect(vm.$refs.compo.picker.$el.querySelector('.ac-time-panel')).to.ok;
        done();
      }, DELAY);
    });
    ...
    it('both picker show correct formated value (extract date-format and time-format from format property)', (done) => {
      ...
      setTimeout((_) => {
        ...
        const dateInput = datePanel.$el.querySelector(
          '.ac-date-picker__time-header > span:nth-child(1) input'
        );
        const timeInput = datePanel.$el.querySelector(
          '.ac-date-picker__time-header > span:nth-child(2) input'
        );
        timeInput.focus();
        setTimeout((_) => {
    ...
    it('click now button', (done) => {
      ...
      setTimeout((_) => {
        ...
        vm.picker.$el.querySelector('.ac-picker-panel__link-btn').click();
        setTimeout((_) => {
    ...
    it('click timepicker', (done) => {
      ...
      setTimeout((_) => {
        const input = vm.picker.$el.querySelectorAll(
          '.ac-date-picker__editor-wrap input'
        )[1];
        triggerEvent(input, 'focus');

        setTimeout((_) => {
          const timePanel = vm.picker.$el.querySelector('.ac-time-panel');
    ...
    it('select time', (done) => {
      ...
      setTimeout((_) => {
        ...
        setTimeout((_) => {
          const button = vm.picker.$el.querySelector('.ac-time-panel .confirm');
          button.click();
      ...
      it('pick date, emits on confirm', (done) => {
        ...
        setTimeout((_) => {
          ...
          setTimeout((_) => {
            ...
            vm.$refs.compo.picker.$el
              .querySelector('.ac-picker-panel__footer .ac-button--default')
              .click();
            setTimeout((_) => {
    ...
    it('now button: can not choose disabled date', (done) => {
      ...
      setTimeout((_) => {
        // click now button
        vm.$refs.compo.picker.$el
          .querySelector('.ac-picker-panel__footer .ac-button--text')
          .click();
        setTimeout((_) => {
          expect(vm.value).to.equal('');
          done();
    ...
    it('select time honors picked date', (done) => {
      ...
      setTimeout((_) => {
        ...
        vm.$refs.compo.picker.$el
          .querySelector('.ac-date-picker__header .ac-icon-arrow-right')
          .click();
        setTimeout((_) => {
          vm.$refs.compo.picker.$el
            .querySelector('.ac-date-picker__header .ac-icon-d-arrow-right')
            .click();
          setTimeout((_) => {
  ...
  describe('type:week', () => {
    ...
    it('click cell', (done) => {
      vm.picker.$el.querySelector('.ac-date-table__row .available').click();
      setTimeout((_) => {
    ...
    it('click year label', () => {
      vm.picker.$el.querySelector('.ac-date-picker__header-label').click();
      expect(vm.picker.currentView).to.equal('year');
    });

    it('click month label', () => {
      vm.picker.$el
        .querySelectorAll('.ac-date-picker__header-label')[1]
        .click();
    ...
    it('select month', (done) => {
      ...
      setTimeout((_) => {
        vm.picker.$el.querySelector('.ac-month-table .cell').click();

        setTimeout((_) => {
          expect(
            vm.picker.$el.querySelector('.ac-date-table.is-week-mode').style
              .display
          ).to.equal('');
          done();
        }, DELAY);
    ...
    it('highlight correctly', (done) => {
      ...
      setTimeout(() => {
        ...
        const numberOfHighlightRows = () =>
          pickerEl.querySelectorAll('.ac-date-table__row.current').length;
        expect(numberOfHighlightRows()).to.equal(0);
        setTimeout(() => {
          pickerEl
            .querySelector(
              '.ac-date-table__row ~ .ac-date-table__row td.available'
            )
            .click();
          setTimeout(() => {
            expect(vm.value).to.exist;
            ...
            setTimeout(() => {
              ...
              pickerEl.querySelector('.ac-icon-arrow-right').click();
              setTimeout(() => {
                expect(numberOfHighlightRows()).to.equal(0);
                // test: next year should not have highlight
                pickerEl.querySelector('.ac-icon-arrow-left').click(); // go back one month
                pickerEl.querySelector('.ac-icon-d-arrow-right').click();
                setTimeout(() => {
                  expect(numberOfHighlightRows()).to.equal(0);
                  done();
                }, DELAY);
  ...
  describe('type:daterange', () => {
    ...
    it('type:daterange unlink:true', (done) => {
      ...
      setTimeout((_) => {
        ...
        setTimeout((_) => {
          ...
          const left = panels[0].querySelector('.ac-date-range-picker__header');
          const right = panels[1].querySelector(
            '.is-right .ac-date-range-picker__header'
          );
    ...
    it('unlink panels', (done) => {
      ...
      setTimeout((_) => {
        ...
        panels[1].querySelector('.ac-icon-d-arrow-right').click();
        panels[1].querySelector('.ac-icon-arrow-right').click();
    ...
    describe('default value', () => {
      it('single', (done) => {
        ...
        setTimeout((_) => {
          ...
          defaultEls[0].click();
          setTimeout((_) => {
            $el.querySelector('.ac-date-table td.default + td').click();
            setTimeout((_) => {
              expect(vm.value).to.eql(expectValue);
              done();
            }, DELAY);
  ...
  describe('type:datetimerange', () => {
    ...
    it('create', () => {
      expect(
        Array.prototype.slice.call(
          vm.picker.$el.querySelectorAll('.ac-time-panel')
        )
      ).to.length(2);
    });
      ...
      setTimeout((_) => {
        ...
        setTimeout((_) => {
          const pickers = compo.picker.$el.querySelectorAll(
            '.ac-date-range-picker__time-header .ac-date-range-picker__editors-wrap'
          );
          const left = {
            dateInput: pickers[0].querySelector(
              '.ac-date-range-picker__time-picker-wrap:nth-child(1) input'
            ),
            timeInput: pickers[0].querySelector(
              '.ac-date-range-picker__time-picker-wrap:nth-child(2) input'
            ),
          };
          const right = {
    ...
    it('prev/next month button', (done) => {
      const leftBtn = vm.picker.$el.querySelector(
        '.is-left .ac-icon-arrow-left'
      );
      const rightBtn = vm.picker.$el.querySelector(
        '.is-right .ac-icon-arrow-right'
      );
      const left = vm.picker.$el.querySelector(
        '.is-left .ac-date-range-picker__header'
      );
    ...
    it('prev/next year button', (done) => {
      const leftBtn = vm.picker.$el.querySelector(
        '.is-left .ac-icon-d-arrow-left'
      );
      const rightBtn = vm.picker.$el.querySelector(
        '.is-right .ac-icon-d-arrow-right'
      );
      const left = vm.picker.$el.querySelector(
        '.is-left .ac-date-range-picker__header'
      );
    ...
    it('input date', (done) => {
      const input = vm.picker.$el.querySelector(
        '.ac-date-range-picker__editors-wrap input'
      );
      const pickers = vm.picker.$el.querySelectorAll(
        '.ac-date-range-picker__content'
      );
    ...
    it('select time', (done) => {
      ...
      setTimeout((_) => {
        const button = vm.picker.$el.querySelector(
          '.ac-date-range-picker__time-picker-wrap .ac-time-panel .confirm'
        );
        button.click();
  ...
  describe('type:monthrange', () => {
    ...
    describe('default value', () => {
      ...
      it('single', (done) => {
        ...
        setTimeout((_) => {
          const $el = vm.$refs.compo.picker.$el;
          const defaultEls = $el.querySelectorAll('.ac-month-table td.default');
          expect(defaultEls.length).to.equal(1);
          defaultEls[0].click();
          setTimeout((_) => {
            $el.querySelector('.ac-month-table td.default + td').click();
            setTimeout((_) => {
              expect(vm.value).to.eql(expectValue);
              done();
            }, DELAY);
  ...
  it('picker-options:shortcuts', (done) => {
    ...
    setTimeout((_) => {
      const shortcut = vm.picker.$el.querySelector(
        '.ac-picker-panel__shortcut'
      );

      expect(shortcut.textContent).to.be.equal('今天');
      expect(vm.picker.$el.querySelector('.ac-picker-panel__sidebar')).to.be.ok;
  ...
  describe('picker-options:selectableRange', () => {
    ...
    it('selectableRange', (done) => {
      ...
      setTimeout(() => {
        ...
        input1.focus();
        setTimeout((_) => {
          const list = $el.querySelectorAll('.ac-time-spinner__list');
  ...
  describe('picker-options:className', () => {
    it('set custom class name', async () => {
      ...
      expect(
        vm.$refs.compo.picker.$el
          .querySelector('.ac-date-table__row td')
