...
describe('Tabs', () => {
  ...
  it('create', (done) => {
    ...
    let paneList = vm.$el.querySelector('.ac-tabs__content').children;
    let spy = sinon.spy();
  ...
  it('active-name', (done) => {
    ...
    setTimeout((_) => {
      const paneList = vm.$el.querySelector('.ac-tabs__content').children;
      const tabList = vm.$refs.tabs.$refs.nav.$refs.tabs;
  ...
  it('card', () => {
    ...
    expect(vm.$el.classList.contains('ac-tabs--card')).to.be.true;
  });
  it('border card', () => {
    vm = createVue(
      {
    ...
    expect(vm.$el.classList.contains('ac-tabs--border-card')).to.be.true;
  });
  it('dynamic', (done) => {
    vm = createVue(
      {
    ...
    setTimeout(() => {
      expect(vm.$el.querySelectorAll('.ac-tab-pane').length).to.equal(4);
      vm.tabs.push({
        label: 'tab5',
        name: 'tab5',
      });
      setTimeout(() => {
        expect(vm.$el.querySelectorAll('.ac-tab-pane').length).to.equal(5);
        done();
      });
    }, 100);
  });
  it('editable', (done) => {
    ...
    setTimeout((_) => {
      ...
      tabList[1].querySelector('.ac-icon-close').click();
      setTimeout((_) => {
        expect(tabList.length).to.be.equal(2);
        expect(paneList.length).to.be.equal(2);
        expect(tabList[1].classList.contains('is-active')).to.be.true;

        vm.$refs.tabs.$el.querySelector('.ac-tabs__new-tab').click();
        setTimeout((_) => {
  ...
  it('addable & closable', (done) => {
    ...
    setTimeout((_) => {
      ...
      setTimeout((_) => {
        ...
        tabList[2].querySelector('.ac-icon-close').click();
        setTimeout((_) => {
          expect(tabList.length).to.be.equal(2);
          expect(paneList.length).to.be.equal(2);
          expect(tabList[1].classList.contains('is-active')).to.be.true;
  ...
  it('closable in tab-pane', (done) => {
    ...
    setTimeout(() => {
      expect(vm.$el.querySelectorAll('.ac-icon-close').length).to.equal(2);
      done();
    }, 100);
  });
  it('disabled', (done) => {
  ...
  it('horizonal-scrollable', (done) => {
    ...
    setTimeout((_) => {
      const btnPrev = vm.$el.querySelector('.ac-tabs__nav-prev');
      btnPrev.click();
      vm.$nextTick((_) => {
        const tabNav = vm.$el.querySelector('.ac-tabs__nav-wrap');
        expect(tabNav.__vue__.navOffset).to.be.equal(0);

        const btnNext = vm.$el.querySelector('.ac-tabs__nav-next');
        btnNext.click();
  ...
  it('should work with lazy', (done) => {
    ...
    expect(
      vm.$el.querySelector('.ac-tabs__content').children.length
    ).to.be.equal(3);
    expect(vm.$el.querySelector('.ac-tabs__content > #pane-D')).to.be.equal(
      null
    );

    setTimeout((_) => {
      vm.$el.querySelector('.ac-tabs__nav > #tab-D').click();
      vm.$nextTick(() => {
        expect(
          vm.$el.querySelector('.ac-tabs__content').children.length
        ).to.be.equal(4);
        expect(
          vm.$el.querySelector('.ac-tabs__content > #pane-D')
        ).not.to.be.equal(null);

        vm.$el.querySelector('.ac-tabs__nav > #tab-A').click();
        vm.$nextTick(() => {
          expect(
            vm.$el.querySelector('.ac-tabs__content').children.length
          ).to.be.equal(4);
