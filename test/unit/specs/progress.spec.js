...
describe('Progress', () => {
  ...
  it('create', () => {
    ...
    expect(
      vm.$refs.percent0.$el.querySelector('.ac-progress__text').innerText
    ).to.be.equal('0%');
    expect(
      vm.$refs.percent0.$el.querySelector('.ac-progress-bar__inner').style.width
    ).to.be.equal('0%');

    expect(
      vm.$refs.percent50.$el.querySelector('.ac-progress__text').innerText
    ).to.be.equal('50%');
    expect(
      vm.$refs.percent50.$el.querySelector('.ac-progress-bar__inner').style
        .width
    ).to.be.equal('50%');

    expect(
      vm.$refs.percent100.$el.querySelector('.ac-progress__text').innerText
    ).to.be.equal('100%');
    expect(
      vm.$refs.percent100.$el.querySelector('.ac-progress-bar__inner').style
        .width
    ).to.be.equal('100%');
  });
  it('status', () => {
    ...
    expect(
      vm.$refs.lineSuccess.$el.querySelector(
        '.ac-progress__text .ac-icon-circle-check'
      )
    ).to.be.exist;
    ...
    expect(
      vm.$refs.lineException.$el.querySelector(
        '.ac-progress__text .ac-icon-circle-close'
      )
    ).to.be.exist;
    ...
    expect(
      vm.$refs.circleSuccess.$el.querySelector(
        '.ac-progress__text .ac-icon-check'
      )
    ).to.be.exist;
    ...
    expect(
      vm.$refs.circleException.$el.querySelector(
        '.ac-progress__text .ac-icon-close'
      )
    ).to.be.exist;
  });
  it('text inside', () => {
    ...
    expect(vm.$el.classList.contains('ac-progress--text-inside')).to.be.true;
  });
  it('stroke width', () => {
    vm = createVue(
      {
    ...
    expect(
      vm.$el.querySelector('.ac-progress-bar__outer').style.height
    ).to.be.equal('8px');
  });
  it('show text', () => {
    vm = createVue(
    ...
    expect(vm.$el.querySelector('.ac-progress__text')).to.not.exist;
  });
  it('circle', () => {
    vm = createVue(
      {
    ...
    expect(vm.$el.classList.contains('ac-progress--circle')).to.be.true;
  });
  it('dashboard', () => {
    vm = createVue(
      {
    ...
    expect(vm.$el.classList.contains('ac-progress--dashboard')).to.be.true;
  });
  it('width', () => {
    vm = createVue(
      {
    ...
    expect(
      vm.$el.querySelector('.ac-progress-circle').style.height
    ).to.be.equal('120px');
    expect(vm.$el.querySelector('.ac-progress-circle').style.width).to.be.equal(
      '120px'
    );
  });
  it('should work with stroke-width', () => {
    ...
    expect(
      vm.$el.querySelector('.ac-progress-bar__innerText').offsetTop
    ).to.be.equal(12);
  });
  it('color', () => {
    vm = createVue(
    ...
    expect(
      vm.$el.querySelector('.ac-progress-bar__inner').style.backgroundColor
    ).to.equal('rgb(0, 0, 0)');
  });
  ...
  it('format content', () => {
    ...
    expect(vm.$el.querySelector('.ac-progress__text').innerHTML).to.equal(
      '占比50%'
    );
  });
});
