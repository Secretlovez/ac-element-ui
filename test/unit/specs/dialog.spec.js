...
describe('Dialog', () => {
  ...
  it('create', (done) => {
    ...
    setTimeout(() => {
      ...
      expect(vm.$el.querySelector('.ac-dialog__title').textContent).to.equal(
        'dialog test'
      );
  ...
  it('render correct content', (done) => {
    ...
    setTimeout(() => {
      const footerBtns = vm.$el.querySelectorAll(
        '.ac-dialog__footer .ac-button'
      );
      expect(
        vm.$el.querySelector('.ac-dialog__body span').textContent
      ).to.equal('这是一段信息');
  ...
  describe('props', () => {
    ...
    it('fullscreen', () => {
      ...
      const dialogEl = vm.$el.querySelector('.ac-dialog');
    ...
    it('top', () => {
      ...
      expect(vm.$el.querySelector('.ac-dialog').style.marginTop).to.equal(
        '100px'
      );
    });
    ...
    it('custom-class', () => {
      ...
      expect(vm.$el.querySelector('.ac-dialog').classList.contains('my-dialog'))
        .to.true;
    });
  });
  ...
  it('click header btn', (done) => {
    ...
    const dialog = vm.$children[0];
    setTimeout(() => {
      dialog.$el.querySelector('.ac-dialog__headerbtn').click();
      setTimeout(() => {
        expect(vm.visible).to.be.false;
        done();
      }, 500);
