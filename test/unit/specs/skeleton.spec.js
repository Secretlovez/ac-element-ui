...
describe('Skeleton.vue', () => {
  ...
  it('render test', () => {
    ...
    const el = vm.$el;
    expect(el.querySelectorAll('.ac-skeleton__p').length).to.equal(4);
    expect(Array.from(el.children[0].classList)).to.contain('ac-skeleton');
  });
  ...
  it('should render x times', async () => {
    ...
    const wrapper = vm.$el;
    expect(wrapper.querySelectorAll('.ac-skeleton__p').length).to.equal(4);

    vm.count = 2;

    await waitImmediate();
    expect(wrapper.querySelectorAll('.ac-skeleton__p').length).to.equal(8);
  });
  ...
  it('should render x rows', () => {
    ...
    expect(vm.$el.querySelectorAll('.ac-skeleton__p').length).to.equal(5);
  });
  ...
  it('should render template slots', () => {
    ...
    expect(vm.$el.querySelector('.ac-skeleton').textContent).to.be.equal(
      content
    );
  });
