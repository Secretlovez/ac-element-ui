import { arrayFindIndex } from '@syhr/ac-element-ui/src/utils/util';
...
import {
  ...
  removeClass,
  addClass,
} from '@syhr/ac-element-ui/src/utils/dom';
import ElCheckbox from '@syhr/ac-element-ui/packages/checkbox';
import ElTooltip from '@syhr/ac-element-ui/packages/tooltip';
...
export default {
  ...
  render(h) {
    ...
    return (
      <table class='ac-table__body' cellspacing='0' cellpadding='0' border='0'>
        <colgroup>
          {this.columns
            .filter(
              (column, index) => !(this.columnsHidden[index] && this.fixed)
            )
            .map((column) => (
  ...
  watch: {
    ...
    'store.states.hoverRow'(newVal, oldVal) {
      ...
      raf(() => {
        const rows = this.$el.querySelectorAll('.ac-table__row');
  ...
  methods: {
    ...
    getRowClass(row, rowIndex) {
      ...
      if (this.stripe && rowIndex % 2 === 1) {
        classes.push('ac-table__row--striped');
      }
    ...
    handleCellMouseEnter(event, row) {
      ...
      if (!(hasClass(cellChild, 'ac-tooltip') && cellChild.childNodes.length)) {
        return;
      }
    ...
    getRowHeight(rowKey) {
      ...
      const height = (this.tableLayout.fixedColumnsBodyRowsHeight || {})[
        rowKey
      ];
    ...
    wrappedRowRender({ row, $index, isSelected, isExpanded }) {
      ...
      if (this.hasExpandColumn && isRowExpanded(row)) {
          ...
          [
            ...
            <tr key={'expanded-row__' + tr.key}>
              <td
                ...
                class='ac-table__cell ac-table__expanded-cell'
              >
        ...
        const tmp = [
          this.rowRender({ row, $index, treeRowData, isSelected, isExpanded }),
        ];
        // 渲染嵌套数据
        if (cur) {
