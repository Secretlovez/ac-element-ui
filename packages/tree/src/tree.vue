<template>
  <div
    class="ac-tree"
    :class="{
      'ac-tree--highlight-current': highlightCurrent,
  ...
  >
    ...
    <div class="ac-tree__empty-block" v-if="isEmpty">
      <span class="ac-tree__empty-text">{{ emptyText }}</span>
    </div>
    <div
      v-show="dragState.showDropIndicator"
      class="ac-tree__drop-indicator"
      ref="dropIndicator"
    ></div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    handleKeydown(ev) {
      ...
      if (currentItem.className.indexOf('ac-tree-node') === -1) return;
  ...
  created() {
    ...
    this.$on('tree-node-drag-over', (event, treeNode) => {
      ...
      const iconPosition = dropNode.$el
        .querySelector('.ac-tree-node__expand-icon')
