<template>
  ...
  >
    ...
    >
      <template slot="suffix">
        <i
          ...
          key="clear"
          class="ac-input__icon ac-icon-circle-close"
    ...
    <div v-if="multiple" class="ac-cascader__tags">
      <el-tag
      ...
      <input
        ...
        type="text"
        class="ac-cascader__search-input"
    ...
    <transition name="ac-zoom-in-top" @after-leave="handleDropdownLeave">
      <div
        v-show="dropDownVisible"
        ref="popper"
        :class="['ac-popper', 'ac-cascader__dropdown', popperClass]"
      >
        <el-cascader-panel
          ref="panel"
          v-show="!filtering"
        ...
        <el-scrollbar
          ...
          v-show="filtering"
          tag="ul"
          class="ac-cascader__suggestion-panel"
          view-class="ac-cascader__suggestion-list"
        ...
        >
          <template v-if="suggestions.length">
            ...
            >
              <span>{{ item.text }}</span>
              <i v-if="item.checked" class="ac-icon-check"></i>
            </li>
          </template>
          <slot v-else name="empty">
            <li class="ac-cascader__empty-text">
...
<script>
import Popper from '@syhr/ac-element-ui/src/utils/vue-popper';
import Clickoutside from '@syhr/ac-element-ui/src/utils/clickoutside';
import Emitter from '@syhr/ac-element-ui/src/mixins/emitter';
import Locale from '@syhr/ac-element-ui/src/mixins/locale';
import Migrating from '@syhr/ac-element-ui/src/mixins/migrating';
import ElInput from '@syhr/ac-element-ui/packages/input';
import ElTag from '@syhr/ac-element-ui/packages/tag';
import ElScrollbar from '@syhr/ac-element-ui/packages/scrollbar';
import ElCascaderPanel from '@syhr/ac-element-ui/packages/cascader-panel';
import AriaUtils from '@syhr/ac-element-ui/src/utils/aria-utils';
import { t } from '@syhr/ac-element-ui/src/locale';
import {
  isEqual,
  isEmpty,
  kebabCase,
} from '@syhr/ac-element-ui/src/utils/util';
import { isUndefined, isFunction } from '@syhr/ac-element-ui/src/utils/types';
import { isDef } from '@syhr/ac-element-ui/src/utils/shared';
import {
  addResizeListener,
  removeResizeListener,
} from '@syhr/ac-element-ui/src/utils/resize-event';
...
export default {
  ...
  methods: {
    ...
    focusFirstNode() {
      this.$nextTick(() => {
        ...
        if (filtering && suggestionPanel) {
          ...
          const firstMenu = popper.querySelector('.ac-cascader-menu');
          firstNode = firstMenu.querySelector(
            '.ac-cascader-node[tabindex="-1"]'
          );
        }
    ...
    updateStyle() {
      ...
      const inputInner = $el.querySelector('.ac-input__inner');

      if (!inputInner) return;

      const tags = $el.querySelector('.ac-cascader__tags');
      let suggestionPanelEl = null;

      if (suggestionPanel && (suggestionPanelEl = suggestionPanel.$el)) {
        const suggestionList = suggestionPanelEl.querySelector(
          '.ac-cascader__suggestion-list'
        );
