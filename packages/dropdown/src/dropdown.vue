...
import ElButtonGroup from '@syhr/ac-element-ui/packages/button-group';
...
export default {
  ...
  watch: {
    ...
    focusing(val) {
      const selfDefine = this.$el.querySelector('.ac-dropdown-selfdefine');
      if (selfDefine) {
        // 自定义
        if (val) {
  ...
  render(h) {
      ...
      triggerElm = (
        <el-button-group>
          ...
          <el-button
            ...
            type={type}
            size={dropdownSize}
            class='ac-dropdown__caret-button'
            disabled={disabled}
          >
            <i class='ac-dropdown__icon ac-icon-arrow-down' />
          </el-button>
        </el-button-group>
      );
    } else {
    ...
    return (
      <div class='ac-dropdown' v-clickoutside={hide} aria-disabled={disabled}>
        {triggerElm}
        {menuElm}
      </div>
    );
