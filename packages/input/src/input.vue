  ...
  >
    <template v-if="type !== 'textarea'">
      ...
      <span class="ac-input__prefix" v-if="$slots.prefix || prefixIcon">
        ...
        <i class="ac-input__icon" v-if="prefixIcon" :class="prefixIcon"> </i>
      </span>
      <!-- 后置内容 -->
      <span class="ac-input__suffix" v-if="getSuffixVisible()">
        <span class="ac-input__suffix-inner">
          <template v-if="!showClear || !showPwdVisible || !isWordLimitVisible">
            <slot name="suffix"></slot>
            <i class="ac-input__icon" v-if="suffixIcon" :class="suffixIcon">
            </i>
          </template>
          <i
            v-if="showClear"
            class="ac-input__icon ac-icon-circle-close ac-input__clear"
            @mousedown.prevent
            @click="clear"
          ></i>
          <i
            v-if="showPwdVisible"
            class="ac-input__icon ac-icon-view ac-input__clear"
        ...
        <i
          class="ac-input__icon"
          v-if="validateState"
          :class="['ac-input__validateIcon', validateIcon]"
        >
        </i>
      </span>
      <!-- 后置元素 -->
...
import merge from '@syhr/ac-element-ui/src/utils/merge';
...
export default {
  ...
  computed: {
    ...
    validateIcon() {
      return {
        validating: 'ac-icon-loading',
        success: 'ac-icon-circle-check',
        error: 'ac-icon-circle-close',
      }[this.validateState];
    },
    textareaStyle() {
  ...
  methods: {
    ...
    calcIconOffset(place) {
      let elList = [].slice.call(
        this.$el.querySelectorAll(`.ac-input__${place}`) || []
      );
      ...
      if (this.$slots[pendant]) {
        el.style.transform = `translateX(${place === 'suffix' ? '-' : ''}${
          this.$el.querySelector(`.ac-input-group__${pendant}`).offsetWidth
        }px)`;
      } else {
